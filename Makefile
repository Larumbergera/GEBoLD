...
export CURRENT_VERSION:=$(shell cat ${ROOTDIR}/gebold/_meta.py  | sed -En "s/^__version__\s*=\s*[ubrf]*['\"]([^'\"]+)['\"].*$$/\1/p")
export CURRENT_USER:=$(shell id -u ${USER}):$(shell id -g ${USER})

# Docker configuration
export IMAGE_NAME:=$(or $(IMAGE_NAME), gebold)
...
env-create: ## (re)create a development environment using tox
	tox -e gebold --recreate
	@echo -e "\r\nYou can activate the environment with:\r\n\r\n$$ source ./.tox/gebold/bin/activate\r\n"
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) gebold || true
...
lint: ## static code analysis with pylint
	pylint --rcfile gebold/.pylintrc gebold

lint-tests: ## static test code analysis with pylint
	pylint --rcfile tests/.pylintrc tests

code-style: ## check code style against PEP8 conventions
	pycodestyle gebold --max-line-length=80

code-style-tests: ## check test code style against PEP8 conventions
...
code-locs: ## display metrics (LOCs, number of comments, etc.)
	radon raw --summary gebold

code-complexity: ## check cyclomatic complexity using radon
	xenon --max-absolute B --max-modules A --max-average A gebold
...
security: ## check source code for vulnerabilities
	@[ "${REPORT_FORMAT}" ] && ( mkdir -p docs/_build/security && bandit -v -r -f ${REPORT_FORMAT} -o docs/_build/security/index.html gebold &> /dev/null ) || true
	bandit -v -r gebold
...
docker-run: ## run the specified command on docker (defaults to /bin/bash)
	docker compose -f docker/dev/compose.yml run --rm -u $(CURRENT_USER) gebold $(or $(COMMAND),  $(MAKE) $(TARGET))

docker-shell: ## drop the user into a shell inside a docker container
	docker compose -f docker/dev/compose.yml run --rm -u $(CURRENT_USER) gebold
...
docker-package: ## package an image as a tar archive
	...
	docker save -o ${ROOTDIR}/dist/gebold-$(IMAGE_VERSION).tar $(IMAGE_NAME):$(IMAGE_VERSION)
